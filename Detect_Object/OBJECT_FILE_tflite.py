...
def get_output_tensor(interpreter, index) -> None:
...
def detect_objects(interpreter, image, threshold) -> list:
    ...
    while (key_detect == 0):
        ...
        if (times == 1):
            ...
            for num in range(len(results)):
                ...
                box_bottom = int(
                    results[num]['bounding_box'][2] * CAMERA_HEIGHT)
                ...
                if (labels[label_id] != ''):    # 框選所有類別
                    ...
                    cv2.rectangle(image_src, (box_left, box_top),
                                  (box_right, box_bottom), (0, 0, 255), 2)
            ...
            cv2.putText(image_src,
                        FPS_text,
                        (int(CAMERA_WIDTH*0.6), int(CAMERA_HEIGHT*0.08)
